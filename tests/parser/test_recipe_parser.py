...
@pytest.mark.parametrize(
    "file,path,python_version,expected_status",
    [
        ("no_skip", "/", ">=3.8", True),
        ("no_skip_multi", "/outputs/1", ">=3.8", True),
        ("skip_w_0py", "/", ">= 3.8", True),
        ("skip_w_1py", "/", " >=3.8", True),
        ("skip_w_2py", "/", " >= 3.9", True),
...
def test_update_skip_statement_python(file: str, path: str, python_version: str, expected_status: bool) -> None:
    """
    ...
    assert parser.update_skip_statement_python(path, python_version) == expected_status
    if expected_status:
