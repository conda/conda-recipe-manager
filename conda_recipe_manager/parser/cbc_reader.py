"""
:Description: Read-only parser that is capable of comprehending Conda Build Configuration (CBC) files.
"""
...
from conda_recipe_manager.parser.types import DEFAULT_VARIANTS, CbcOutputType, GeneratedVariantsType, RecipeReaderFlags
from conda_recipe_manager.types import (
    PRIMITIVES_TUPLE,
    JsonType,
    Primitives,
    SentinelType,
...
class CbcReader(RecipeReader):
    """
    Parses an unrendered Conda Build Configuration (CBC) file and provides querying/read-only capabilities. Often these
    files are named `conda_build_configuration.yaml` or `cbc.yaml`

    This work is based off of the `RecipeReader` class. The CBC file format happens to be similar enough to
    the recipe format (with commented selectors) that we can save on some maintenance costs.
    """
    ...
    @staticmethod
    def _construct_default_variants_cbc() -> CbcReader:
        """
    ...
    def generate_cbc_values(cbc_files: list[CbcReader], build_context: BuildContext) -> CbcOutputType:
        ...
        default_variants_cbc = CbcReader._construct_default_variants_cbc()
        ...
        CbcReader._validate_zip_keys_against_cbc_values(zip_keys, cbc_values)
    ...
    @staticmethod
    def generate_variants(cbc_files: list[CbcReader], build_context: BuildContext) -> GeneratedVariantsType:
        """
        ...
        """
        cbc_values, zip_keys = CbcReader.generate_cbc_values(cbc_files, build_context)
